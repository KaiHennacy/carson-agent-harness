...
ARTIFACT_ID="CARSON_LTE_ACCESSIBILITY_FINAL_FRESH_E2E_V29_20260816_161900"
WORK="$HOME/.carson_ui_lte_poc"
CLIENT="$WORK/carson-ui"

CLAUDE_PKG="com.anthropic.claude"
DOCS_PKG="com.google.android.documentsui"
TEST_MARKER="CARSON_UI_LOOP_ACCEPTANCE_V12"
TASK_TEXT="Create one downloadable Termux Bash .sh file that prints exactly CARSON_UI_LOOP_ACCEPTANCE_V12 followed by a newline and exits with status 0. The file must require no input and make no persistent changes."
...
REPORT="$PUBLIC_A/CARSON_LTE_ACCESSIBILITY_FINAL_FRESH_E2E_V29_REPORT.txt"
TMPBASE="${TMPDIR:-$HOME/.cache}/carson-lte-accessibility-v29-$$"
...
say "CARSON_LTE_ACCESSIBILITY_FINAL_FRESH_E2E_V29"
...
say "=== RETIRE COMPLETED V27/V28 ARM / ARM GENUINELY FRESH CARSON TURN ==="
if [[ -f "$CURSOR" ]]; then
    ...
    rm -f "$CURSOR"
    say "COMPLETED_V27_CURSOR_REMOVED=YES"
else
